  ...
  try {
    ...
    const [suppliers, accounts] = await Promise.all([
      ...
      prisma.supplierAccount.findMany({
        where,
        select: {
          supplierId: true, originalAmount: true, pendingAmount: true,
    ...
    for (const acc of accounts) {
      if (!map[acc.supplierId]) map[acc.supplierId] = { pendingAmount: 0, accounts: [] };
      map[acc.supplierId].pendingAmount += Number(acc.pendingAmount);
      map[acc.supplierId].accounts.push(acc);
    }
      ...
      if (remaining > 0) {
        const creditAccount = await tx.supplierAccount.create({
          data: {
            ...
            description: `Crédito a favor del proveedor por sobrepago${description ? ` (${description})` : ''}`,
          },
        });
...
exports.closeSupplierPeriod = async (req, res) => {
  try {
    const { supplierId, description } = req.body;
    const tenantId = req.user.tenantId;

    const marker = await prisma.supplierAccount.create({
      data: {
        tenantId, supplierId,
